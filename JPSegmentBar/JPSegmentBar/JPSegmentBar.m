...
@property (nonatomic, strong) JPSegmentBarConfig *config;

@end

@implementation JPSegmentBar
...
+ (instancetype)segmentBarWithFrame: (CGRect)frame {
    JPSegmentBar *segmentBar = [[JPSegmentBar alloc] initWithFrame:frame];
    // 添加内容承载视图
    return segmentBar;
}
...
- (void)layoutSubviews {
    ...
    if (self.isDivideEqually) {
        ...
        self.indicatorView.width = btn.width - 2;
        self.indicatorView.centerX = btn.centerX;
        self.indicatorView.height = self.config.indicatorHeight;
...
- (JPSegmentBarConfig *)config {
    if (!_config) {
        _config = [JPSegmentBarConfig defaultConfig];
    }
    return _config;
}
